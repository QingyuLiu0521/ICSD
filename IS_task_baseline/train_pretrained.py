...
def generate_durations(config_data, test_only=False, evaluation=False):
    if not evaluation:
...
def prepare_run(argv=None):
    parser = argparse.ArgumentParser("Training a SED system for IS Task")
    parser.add_argument(
        "--conf_file",
    ...
    parser.add_argument(
        "--log_dir",
        default="./exp/IStask_baseline_pretask",
    ...
    generate_durations(configs["data"], test_only, evaluation)
