...
class SEDTask4(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_indx):
        ...
        mask_synth = (
            torch.tensor(
                [
                    str(Path(x).parent)
                    == str(Path(self.hparams["data"]["strong_val_folder"]))
                    for x in filenames
                ]
            )
            .to(audio)
        ...
        if torch.any(mask_synth):
            ...
            filenames_synth = [
                x
                for x in filenames
                if Path(x).parent == Path(self.hparams["data"]["strong_val_folder"])
            ]
    ...
    def validation_epoch_end(self, outputs):
        ...
        intersection_f1_macro_student = compute_per_intersection_macro_f1(
            self.val_buffer_student_synth,
            self.hparams["data"]["strong_val_tsv"],
            self.hparams["data"]["strong_val_dur"],
        )
        synth_student_event_macro = log_sedeval_metrics(
            self.val_buffer_student_synth[0.5], self.hparams["data"]["strong_val_tsv"],
        )[0]
        ...
        synth_teacher_event_macro = log_sedeval_metrics(
            self.val_buffer_teacher_synth[0.5], self.hparams["data"]["strong_val_tsv"],
        )[0]
