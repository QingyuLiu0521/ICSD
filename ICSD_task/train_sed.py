...
def generate_durations(config_data):
    for base_set in ["strong_val", "test"]:
        if not os.path.exists(config_data[base_set + "_dur"]):
...
def single_run(
    ...
    if test_state_dict is None:
        ...
        synth_df = pd.read_csv(config["data"]["synth_train_tsv"], sep="\t")
        synth_set = StronglyAnnotatedSet(
            config["data"]["synth_train_folder"],
            synth_df,
            encoder,
            pad_to=config["data"]["audio_max_len"],
        )

        print(config["data"]["synth_train_tsv"])
        print(config["data"]["synth_train_folder"])
        ...
        if strong_real:
            ...
            print(config["data"]["strong_tsv"])
            print(config["data"]["strong_folder"])
        ...
        synth_df_val = pd.read_csv(config["data"]["strong_val_tsv"], sep="\t")
    ...
    if test_state_dict is None:
        ...
        trainer.fit(icsd_training, ckpt_path=checkpoint_resume)
        best_path = trainer.checkpoint_callback.best_model_path
        print(f"best model: {best_path}")
        test_state_dict = torch.load(best_path)["state_dict"]

    icsd_training.load_state_dict(test_state_dict) 
    trainer.test(icsd_training)

def prepare_run(argv=None):
    parser = argparse.ArgumentParser("Training a SED system for ICSD Task")
    parser.add_argument(
        "--conf_file",
    ...
    generate_durations(configs["data"])
    return configs, args, test_model_state_dict

if __name__ == "__main__":

    # prepare run
    configs, args, test_model_state_dict = prepare_run()
